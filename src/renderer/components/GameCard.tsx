...
import { Play, Heart, Folder, Trash2, Download, Clock, Star, Link, RefreshCw, Settings, Monitor, AlertCircle } from 'lucide-react';
    ...
    <div className="game-card bg-gray-800 rounded-lg overflow-hidden shadow-lg flex flex-col h-full min-h-[400px]">
      <div className="relative">
        ...
        <div className="absolute top-2 right-2 flex space-x-1">
          {hasUpdate && (
            <div className="bg-yellow-500 rounded-full p-1" title={`Update available: ${updateInfo?.newVersion || 'New version'}`}>
      ...
      <div className="p-4 flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-2">
          {game.engine && (
            <div className="text-xs text-blue-400">{game.engine.name}</div>
          )}
        ...
        <div className="flex items-center justify-between mb-2">
          <span className={`text-xs px-2 py-1 rounded ${
            ...
            game.status === 'Ongoing' ? 'bg-blue-600 text-white' :
            game.status === 'Abandoned' ? 'bg-red-600 text-white' :
            game.status === 'Onhold' ? 'bg-yellow-600 text-black' :
