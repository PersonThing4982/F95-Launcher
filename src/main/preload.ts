...
const api = {
  ...
  games: {
    ...
    getByTag: (tag: string, page?: number) => 
      ipcRenderer.invoke('games:by-tag', tag, page),
  ...
  library: {
    ...
    linkF95: (gameId: number, url: string) => 
      ipcRenderer.invoke('library:link-f95', gameId, url),
    syncMetadata: (gameId: number) => 
      ipcRenderer.invoke('library:sync-metadata', gameId),
  ...
  launcher: {
    ...
    detectInstalled: (path: string) => 
      ipcRenderer.invoke('launcher:detect-installed', path)
  },
